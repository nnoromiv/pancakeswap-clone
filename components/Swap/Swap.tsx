...
const Chart = () => {
  ...
  return (
    <div className='flex flex-col p-8 mb-10 border-solid border-[1px] border-card-shadow rounded-xl bg-card-background shadow-card h-[500px] w-full max-w-[50rem] max-md:h-[600px] max-md:px-3 transition-opacity ease-in-out'>
    <div className='flex justify-between gap-2'>
      <div>
        <h3 className='font-semibold text-xl text-secondary-lighter'><span className='font-semibold text-4xl text-secondary-darker'>{randomPrice} </span> BNB/CAKE</h3>
        <p className='font-normal text-sm text-secondary'>{gottenDate}</p>
      </div>
      <div className='flex w-fit h-8 ml-auto bg-primary-darker border-solid border-[1px] border-card-shadow rounded-2xl'>
